...
\begin{document}
	...
	\author*[1]{\fnm{Roman} \sur{Romanik}}\email{romanik@icmp.lviv.ua}
	...
	\abstract{A double-occupancy cell model of a fluid with the Curie-Weiss interaction is studied. First, it is shown that the model is isomorphic to the Blume-Capel model on a complete graph. The phase diagram is obtained using a combination of analytical and numerical methods. The analysis reveals a rich variety of thermodynamic behavior depending on the repulsion coupling strength. In particular, the model exhibits a regime resembling the single-occupancy lattice gas, a region with two coexistence lines terminating at two distinct critical points, and an interval of the repulsion coupling where a triple point emerges. The coordinates of the critical points and triple points are calculated over a wide range of repulsion coupling values, and the tricritical point is identified.}
	...
	Soft matter physics extensively relies on the soft-core, or bounded, potentials of interaction~\cite{Likos01}. They are considered as effective potentials for interactions between complex macromolecules in solutions. Examples are the solutions of polymer chains~\cite{LBHM00,BLHM01}, dendrimers~\cite{LSLetal01,MKL08}, star polymers~\cite{LLWetal98}, and other so-called ultrasoft colloids~\cite{Likos06}. The effect of interpenetrability of the molecules leads to interesting physical phenomena that are not observed in simple fluids with hard-core short-range interactions. Examples of such phenomena are the reentrant melting~\cite{LWL98} and associated high-density fluids~\cite{LLWL00}, clustered crystalline phases~\cite{MGKNL06,MCF07,MKL08} and cluster fluids~\cite{LMLKB10}. The phase diagrams of such systems of soft particles may exhibit multiple first-order phase transition lines each terminating at the corresponding critical points~\cite{NL11, WS13, WS14, Prestipino14, PGT15, dMDM23}. Therefore, there may be multiple, sometimes infinite, number of critical points present in the phase diagram. The possibility of particle interpenetration in systems with bounded interactions naturally motivates coarse-grained lattice descriptions allowing multiple occupancy of lattice cells. Such models provide a simplified framework for studying thermodynamic effects associated with ultrasoft effective interactions, including clustering phenomena and complex phase behavior.
	
	Lattice gas models have played an important role in understanding different phenomena in condensed matter physics~\cite{Baxter82,FriedliVelenik17}. Lattice models useful in the area of soft matter physics with bounded interactions must naturally allow multiple occupancy of cells by particles. Indeed, the lattice gases with multiple occupancy have recently gained more attention due to development in the soft matter physics and application of bounded effective potentials to describe the interactions between complex macromolecules that can interpenetrate with a finite energy penalty. The lattice models with unrestricted occupancies were studied in \cite{FHL04}, \cite{FL18,FL20}, and by us in~\cite{KKD20,KD22,RDKPS26a}. A particularly simple realization of a multiple-occupancy lattice model is the double-occupancy case. Such model was considered in~\cite{LYZ21}, where it was studied in the two-dimensional case with nearest-neighbors interactions. In particular, it was shown in~\cite{LYZ21} that the model is isomorphic to the Blume-Capel model~\cite{Blume66,Capel66}, similar to how the single-occupancy lattice gas is isomorphic to the Ising model~\cite{LY52}.
	
	In the current work, we study the double-occupancy cell model of a fluid with Curie-Weiss interaction. First, we show that the model can be obtained from the Blume-Capel model on the complete graph~\cite{HKBHK25} by a simple transformation. Then, we investigate its thermodynamic behavior in the framework of the grand canonical ensemble using the methods developed in~\cite{KKD20,KD22,RDKPS26a} for the infinite-occupancy cell model with Curie-Weiss interaction. Depending on the ratio between the repulsion and attraction couplings, the system exhibits qualitatively different types of phase behavior, including regimes with one or two critical points, tricritical behavior, and triple-point formation. We calculate the coordinates of the critical and triple points over a wide range of repulsion strengths and analyze the corresponding phase diagrams. Therefore, the present model provides a minimal mean-field framework for investigating phase transitions and coexistence phenomena in multiple-occupancy lattice systems.
	
	\section{Double occupancy cell fluid model with Curie-Weiss-type interaction}
	We consider an open system of point particles in a macroscopic three-dimensional region ${\mathcal V} \subset \mathbb R^3$ of volume $V = \abs{\mathcal{V}}$. The region $\mathcal{V}$ is partitioned into $\mathscr{N}$ non-overlapping congruent cubic cells each of volume $v=\sigma^d$, where $\sigma$ is a linear length, and $d$ is the number of space dimensions (we set $d=3$ from the beginning). The volume $V$ of the whole system is thus $V = v \mathscr{N}$. In the thermodynamic limit, $V \to \infty$, $\mathscr{N} \to \infty$, while $\lim_{V \to \infty; \mathscr{N} \to \infty}v = \lim_{V \to \infty; \mathscr{N} \to \infty} V/\mathscr{N} = const$. The maximum number of particles allowed to occupy a cell is $n_{\rm max} = 2$. Hence, the model is called the double-occupancy cell fluid model. In general, this model is isomorphic to the Blume-Capel model, and different interparticle interactions can be considered. In~\cite{LYZ21}, the case of nearest-neighbors attractions and in-cell repulsion was studied in two dimensions by the means of Monte Carlo simulations. A case of a quantum system was studied in~\cite{Mancini05}, where the extended Hubbard model in the ionic limit was shown to also be isomorphic to the Blume-Capel model. 
	...
	\end{equation}
	Here, terms of order $J/(2\mathscr{N})$ relative to $\Delta$, and $J/\mathscr{N}$ relative to $(J-2\Delta)$, have been neglected.
	...
	\end{eqnarray}
	Usually, when lattice gas models are considered, the terms of type $(v/\Lambda)^{n_l}$ are not included into the partition function, and only configuration partition functions are studied~\cite{FriedliVelenik17}. We, however, consider a cell model as a coarse-grained model for a continuum system. Therefore, $\Lambda^3$ appears due to the integration over momenta, and $v$ due to transition from the integration over space variables to the summation over lattice cells
	\begin{equation}
		\int_V {\rm d} {\vb{r}} \to v \sum_{l=1}^{\mathscr{N}} \qquad.
	\end{equation}
	The issue of inclusion of the term $\frac{1}{\prod_l n_l!}$ into the definition of $\Xi$ was discussed in~\cite{FL18}. It was argued that the inclusion of this term corresponds to the case of distinguishable particles, while not including such term corresponds to the system of indistinguishable particles. The grand partition function for indistinguishable particles is therefore expressed as
	\begin{eqnarray}
		\label{Xi:indist}
	...
	\end{eqnarray}
	The comparison of the results for the two cases is of great interest as well. For example, in Refs.~\cite{FL18, FL20} the two cases were studied alongside throughout both papers. In this paper, we consider the case of the system of distinguishable particles in more details, since it was the system we implied in our previous works. However, in Section~\ref{sec:indist} we present the result for the system of indistinguishable particles as well. As will be seen there, the latter case may be of more importance in the context of isomorphism of the double-occupancy cell model to the Blume-Capel model.
	
	\section{Distinguishable particles}\label{sec:distinct}
	
	In this Section, we primarily focus on the case of distinguishable particles.
	The explicit expression for the grand partition function defined in~\eqref{eq:Xi_pi} becomes
	\begin{eqnarray}
		\label{eq:Xi1}
	...
	\end{eqnarray}
	where $v^* = v/\Lambda_J^3$, $\Lambda_J = (2\pi \hbar^2/mJ)^{1/2}$, and
	\begin{equation}
		\pi(T^*,\mu^*; n_l) = \frac{\left(v^* T^{*3/2}\right)^{n_l}}{n_l!} \exp\left(\frac{\mu^* + 2a -1}{T^*}n_l - \frac{a}{T^*} n_l^2\right).
	\end{equation}
	The last two equations are the same as obtained in~\cite[see Eq.~(2.11)]{KKD20} for the infinite-occupancy cell model, with three differences. First, the summation by the occupancy numbers is now restricted by the maximum occupancy number $n_{\rm max}=2$. Second, the chemical potential is shifted by value $2a-1$. Third, the coefficient at $n_l^2$ is now $a/T^*$, but was $a/(2T^*)$. Nevertheless, the methodology developed in~\cite{KKD20,KD22,RDKPS26a} can be applied here as well. 
	Therefore, we transformed the Blume-Capel model on a complete graph to a double-occupancy cell fluid model with the Curie-Weiss interaction. This illustrates that there exist an isomorphism between the two models. 
	
	There exists one more crucial difference between the infinite-occupancy and double-occupancy models for the considered case of long-ranged, Curie-Weiss-type interaction. The condition of stability $a > 1$~\cite{KKD20} is not required anymore, since the thermodynamic catastrophe~\cite{FR66,HR07,FL20} is easily avoided by a finite maximum occupancy number $n_{\rm max}=2$. This allows us to consider a system in a wider range of parameter $a$. In fact, there are not any restrictions on $a$ in the double-occupancy model, and it cat take on any real values. In this paper, we will consider only non-negative values $a \geq 0$.
	
	Let us now explicitly apply the Gaussian integral
	\begin{equation}
	...
	\end{equation}
	to Eq.~\eqref{eq:Xi1} and introduce the ``effective'' (or ``shifted'') chemical potential $\mu_{\rm eff}^*$ as
	\begin{equation}
		\mu_{\rm eff}^* = \mu^* + 2a -1,
	\end{equation}
	...
	\end{equation}
	with special functions $\tilde{K}_0(T^*,\mu_{\rm eff}^*;y)$ defined as
	\begin{equation}
	...
	\begin{equation}
		E_j(T^*,\mu^*_{\rm eff}; z) \equiv \frac{\partial^*}{\partial z^j} E(T^*, \mu^*_{\rm eff}; z).
	\end{equation}
	...
	\begin{figure}
		...
		\caption{The $T^* - \rho^*$ phase diagrams of the double-occupancy lattice gas for the case of distinguishable particles at different values of $a=\Delta/J$.}
	...
	For large enough $a$, and for low enough temperatures, three distinct phases exist in the studied double-occupancy cell model of a fluid. By analogy with our previous works~\cite{KD22}, we call these phases phase I, phase II, and phase III, respectively. However, we can also adopt the terminology of other works where two critical points are observed in a fluid. For example, gas (phase I), low-density liquid (phase II), and high-density liquid (phase III) terminology was adopted in~\cite{Franzese07}. In~\cite{LYZ21}, the corresponding phases were called gas, liquid, and crystal states. We, however, avoid calling the dense fluid phase as a crystal state, since we deal rather with high-density mean-field fluids, similar to the one reported in~\cite{LBH00}. See also Appendix~\ref{sec:pair} where this idea is supported by the results for the pair distribution function.
	
	\section{Conclusions}
	We studied a double-occupancy cell model of a many-particle system, which means that no more than two particles can be located in the same cell simultaneously. The interaction between particles is considered in the form of two contributions: long-range, global attraction in the form of Curie-Weiss interaction, and local finite repulsion between two particles in the same cell. Even though the model is of interest by itself, in this paper we first obtain the hamiltonian of it from the Blume-Capel model on a complete graph, via a simple transformation from spin variables $\sigma_l \in \{-1, 0, +1\}$ to occupancy variables $n_l \in \{0, 1, 2\}$. This transformation reveals the existing isomorphism between the Blume-Capel model and the double-occupancy cell model. When studying the multiple-occupancy cell fluid (lattice gas) models, two cases are considered, that of distinguishable and indistinguishable particles. Mathematically, the difference lies in the definition of the grand partition functions for the two cases, compare Eq.~\eqref{eq:Xi_pi} and Eq.~\eqref{Xi:indist}. In the article we present how the grand partition function for the double-occupancy cell model with the Curie-Weiss-type interaction is calculated based on a method previously developed for the infinite-occupancy cell model. We present more details for the case of distinguishable particles. In particular, we obtain coordinates of the critical points for a wide range of parameter $a$, $0 \leq a \leq 5.0$. We find the interval where a triple point exist, $\frac{1}{2}\ln 2 < a < 0.5$, and calculate the coordinates of the triple points. Then, we identify the tricritical point, where the densities of three phases -- phase I, phase II, and phase III -- become the same. For any value of $a$ such that $a< \frac{1}{2}\ln 2$, there is one critical point, while at $a > \frac{1}{2}\ln 2$, there are two critical points of the same temperature, but different densities.
	We build the phase diagram in the $T^* - \rho^* - a$ space, where we show the lines of critical points, as well as the line of triple points. All lines meet at the tricritical point. The coexistence curves are presented in the $T^*-\rho^*$ plane for a few representative values of $a$, such that we cover regions with one critical point, the region with two critical points, and the case where a triple point exists.
	
	The phase behaviour of the cell model for the system of indistinguishable particles is qualitatively the same as in the case of distinguishable ones. Quantitatively, however, the case of indistinguishable particles precisely reproduces the results of the Blume-Capel model at some limiting cases, where the results for the system of distinguishable particles differs. First, at $a=0$, the critical temperature $T^*_c=\frac{2}{3}$ is obtained, in contrast to $T^*_c=0.586$ in the case of distinguishable particles. Second, the coordinates of the tricritical point $(T^*_{\rm tc} = \frac{1}{3}; a_{\rm tc} = \frac{2}{3}\ln 2)$ are the same as in the Blume-Capel model on a complete graph. This result leads to a conclusion that the double-occupancy cell model is better suited as the isomorphic one to the Blume-Capel model in the case of indistinguishable particles.
	
	The double-occupancy cell model is a useful model in statistical mechanics for studying the systems of particles interacting via bounded potentials; in soft matter physics, where the effective interactions are often bounded; and in the theory of lattice gases, where multiple occupancy has been paid much less attention than single-occupancy.
	...
	\begin{figure}[htbp]
		...
		\caption{\label{fig:g2_r} Pair distribution function $g^{(2)}({\vb r}_1, {\vb r}_2)$ as a function of separation $r = \abs{{\vb r}_1 - {\vb r}_2}$. The quantity $r^*$ denotes distance from the point with coordinate ${\vb r}_1$ to the boundary of the cubic cell containing ${\vb r}_1$, in the direction of ${\vb r}_2$. The results are presented for the system of distinguishable particles, at $a=1.0$. Phase I: $T^* = 0.25$, $\rho^*=0.1$; Phase II: $T^* = 0.2$, $\rho^* = 0.5$; Phase III: $T^*_c = 0.25$, $\rho^* = 1.9$; Supercritical: $T^* = 0.5$, $\rho^* = 0.5$. At $r > r^*$, $g^{(2)} = 1$.}
	\end{figure}
	
	In Fig.~\ref{fig:g2_r}, the spatial dependency of $g^{(2)} (\vb{r}_1, \vb{r}_2)$, Eq.~\eqref{g2_r}, is illustrated for the system of distinguishable particles at $a=1.0$. As is seen from the plot, $g^{(2)}$ at any selected $T^*$ and $\rho^*$ is presented as a step function. While $\vb{r}_1$ and $\vb{r}_2$ are located within one cell, $g^{(2)}$ takes on a value that is dependent on temperature and density. However, as soon as the coordinates of two particles belong to different cells, the pair distribution function becomes unity. At high densities, this is known as high-density mean field fluid, the behavior which is also observed in systems with soft interactions~\cite{LBH00}.
	
	To present the results in Fig.~\ref{fig:g2_r}, we have fixed $a=1.0$ and selected the following representative states. For phase I, $T^*=0.25$ and $\rho^*=0.1$; for phase II, $T^*=0.2$ and $\rho^*=1.0$; for phase III, $T^*=0.25$ and $\rho^*=1.9$; and for supercritical state, $T^*=0.5$ and $\rho^*=0.5$. As expected, the pair distribution function at a given temperature increases with density.
	...
	\bibliography{articles,books,online_urls}
...
\begin{eqnarray*}
	&& Q_{T^*,\mu^*}(n) = \frac{\left(T^{*3/2}\right)^n}{K_0(T^*; \bar{z}_{\rm max}) n!} \times
	\nonumber\\
	&& \times \exp\left(\bar{z}_{\rm max}n - \frac{a}{2T^*}n^2 \right),
	\quad n \in \{0,1,2\},
\end{eqnarray*}
